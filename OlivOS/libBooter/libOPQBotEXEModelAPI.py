...
class server(OlivOS.API.Proc_templet):
    ...
    def run(self):
        if self.Proc_data['bot_info_dict'].platform['model'] in ['opqbot_auto', 'opqbot_port', 'opqbot_port_old']:
        ...
        while self.flag_run:
            ...
            if not os.path.exists('./lib/OPQBot.exe'):
                self.log(
                    ...
                    OlivOS.L10NAPI.getTrans(
                        'OlivOS libOPQBotEXEModel server [{0}] can`t found target lib', [self.Proc_name], modelName
                    ),
                )
                break
            releaseDir('./conf')
            ...
            copyFile(
                src='./lib/OPQBot.exe', dst='./conf/OPQBot/' + self.Proc_data['bot_info_dict'].hash + '/OPQBot.exe'
            )
            if self.Proc_data['bot_info_dict'].platform['model'] in [
                ...
                threading.Thread(target=self.check_model_flag, args=(), daemon=self.deamon).start()
                self.get_model_stdout(model_Proc)
                # model_Proc.communicate(timeout = None)
                self.log(
                    3,
                    OlivOS.L10NAPI.getTrans(
                        'OlivOS libOPQBotEXEModel server [{0}] will retry in 10s...', [self.Proc_name], modelName
                    ),
                )
                self.Proc_data['model_Proc'] = None
                time.sleep(8)
            elif self.Proc_data['bot_info_dict'].platform['model'] in ['opqbot_port_old']:
                self.log(
                    2,
                    OlivOS.L10NAPI.getTrans(
                        'OlivOS libOPQBotEXEModel server [{0}] will run under visiable mode',
                    ...
                    if 'input' == rx_packet_data.key['data']['action']:
                        if 'data' in rx_packet_data.key['data']:
                            ...
                            self.log(
                                2,
                                log_data,
                                [(self.getBotIDStr(), 'default'), ('OPQBot', 'default'), ('stdin_send', 'default')],
                            )
    ...
    def get_model_stdout(self, model_Proc: subprocess.Popen):
        for line in iter(model_Proc.stdout.readline, b''):
            try:
                ...
                self.send_log_event(log_data)
                self.log(1, log_data, [(self.getBotIDStr(), 'default'), ('OPQBot', 'default'), ('onebot', 'default')])
    ...
    def send_init_event(self):
        self.sendControlEventSend(
            'send',
            {
                ...
                'data': {'action': 'opqbot', 'event': 'init', 'hash': self.Proc_data['bot_info_dict'].hash},
            },
        )
