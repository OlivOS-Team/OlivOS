        ...
        while self.flag_run:
            ...
            if not os.path.exists('./lib/ComWeChat-Client.exe'):
                self.log(
                    ...
                    OlivOS.L10NAPI.getTrans(
                        'OlivOS libCWCBEXEModel server [{0}] can`t found target lib', [self.Proc_name], modelName
                    ),
                )
                break
            releaseDir('./conf')
            ...
            if self.Proc_data['bot_info_dict'].platform['model'] in ['ComWeChatBotClient']:
                self.log(
                    2,
                    OlivOS.L10NAPI.getTrans(
                        'OlivOS libCWCBEXEModel server [{0}] will run under visiable mode', [self.Proc_name], modelName
                    ),
                )
                # self.clear_qrcode()
                # self.Proc_data['check_qrcode_flag'] = False
                ...
                self.log(
                    3,
                    OlivOS.L10NAPI.getTrans(
                        'OlivOS libCWCBEXEModel server [{0}] will retry in 10s...', [self.Proc_name], modelName
                    ),
                )
                self.Proc_data['model_Proc'] = None
                time.sleep(8)
        ...
        for line in iter(model_Proc.stdout.readline, b''):
            try:
                ...
                self.log(
                    1,
                    log_data,
                    [(self.getBotIDStr(), 'default'), ('ComWeChatBotClient', 'default'), ('onebot', 'default')],
                )
    ...
    def send_init_event(self):
        self.sendControlEventSend(
            'send',
            {
                ...
                'data': {'action': 'ComWeChatBotClient', 'event': 'init', 'hash': self.Proc_data['bot_info_dict'].hash},
            },
        )
