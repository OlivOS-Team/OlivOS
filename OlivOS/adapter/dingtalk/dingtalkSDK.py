    ...
    def _do_api(self, req_type, body=None, headers=None, *, flag_body_json=True, request_args=None):
        try:
            ...
            if req_type == 'POST':
                ...
                msg_res = req.request(
                    'GET', send_url, headers=headers, proxies=OlivOS.webTool.get_system_proxy(), **request_args
                )
...
class API:
    ...
    class uploadMedia(api_templet):
        ...
        def do_api(self, req_type='POST', proxy=None):
            ...
            requset_args = {
                ...
                'files': {'media': (self.data.mediaName, self.data.media, self.data.contentType)},
            }

            body = {'type': self.data.mediaType}
            return self._do_api(
                req_type='POST', body=body, headers=self.NO_DATA, flag_body_json=False, request_args=requset_args
            )
    ...
    class sendGroupMessage(api_templet):
        ...
        def do_api(self, req_type='POST', proxy=None):
            if self.data is None:
                return None
            tmp_payload_dict = {'msgKey': self.data.msgKey, 'msgParam': self.data.str_msgParam}
...
def get_Event_from_SDK(target_event):
    ...
    if target_event.sdk_event.payload.data.type in [
        ...
        message_obj = OlivOS.messageAPI.Message_templet('olivos_para', message_para_list)
        try:
        ...
        if message_obj.active:
            ...
            if data_raw['conversationType'] == '1':
                ...
                target_event.data = target_event.private_message(str(sender_id), message_obj, 'private')
                ...
                target_event.data = target_event.group_message(
                    str(data_raw['conversationId']), str(sender_id), message_obj, 'group'
                )
                ...
                if send_type == 'group':
                    ...
                    api_obj.set_data(
                        API.sendGroupMessage.data_T(
                            msgKey='sampleText', msgParam={'content': res_this}, openConversationId=target_id
                        )
                    )
                    api_obj.do_api()
                    ...
                    api_obj.set_data(
                        API.sendPrivateMessage.data_T(
                            msgKey='sampleText', msgParam={'content': res_this}, userIds=[target_id]
                        )
                    )
                    api_obj.do_api()
    ...
    try:
        ...
        msg_upload_api.set_data(
            msg_upload_api.data_T(
                media=pic_file, mediaType='image', contentType=pic_mime, mediaName=uuid.uuid4().hex + '.' + pic_ext
            )
        )
