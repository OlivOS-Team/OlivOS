...
class server(OlivOS.API.Proc_templet):
    ...
    def run(self):
        ...
        while True:
            api_obj = OlivOS.qqGuildv2SDK.API.getGateway(
                OlivOS.qqGuildv2SDK.get_SDK_bot_info_from_Plugin_bot_info(self.Proc_data['bot_info_dict'])
            )
            try:
                api_obj.do_api('GET')
    ...
    def on_message(self, ws, message):
        try:
            # print(message)
            tmp_data_rx_obj = OlivOS.qqGuildv2SDK.PAYLOAD.rxPacket(data=json.loads(message))
    ...
    def run_pulse(self):
        ...
        while self.Proc_data['extend_data']['pulse_interval'] is not None:
            ...
            tmp_data = OlivOS.qqGuildv2SDK.PAYLOAD.sendHeartbeat(self.Proc_data['extend_data']['last_s']).dump()
            if (
                tmp_ws_item != self.Proc_data['extend_data']['ws_item']
