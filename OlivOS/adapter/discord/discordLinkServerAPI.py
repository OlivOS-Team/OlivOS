...
class server(OlivOS.API.Proc_templet):
    ...
    def run(self):
        ...
        while True:
            api_obj = OlivOS.discordSDK.API.getGateway(
                OlivOS.discordSDK.get_SDK_bot_info_from_Plugin_bot_info(self.Proc_data['bot_info_dict'])
            )
            try:
                api_obj.do_api('GET')
    ...
    def on_message(self, ws, message):
        try:
            tmp_data_rx_obj = OlivOS.discordSDK.PAYLOAD.rxPacket(data=json.loads(message))
    ...
    def run_pulse(self):
        ...
        while self.Proc_data['extend_data']['pulse_interval'] is not None:
            ...
            tmp_data = OlivOS.discordSDK.PAYLOAD.sendHeartbeat(self.Proc_data['extend_data']['last_s']).dump()
            if (
                tmp_ws_item != self.Proc_data['extend_data']['ws_item']
    ...
    for bot_info_dict_this in bot_info_dict:
        ...
        if bot_info_dict[bot_hash].platform['sdk'] == 'discord_link':
            ...
            try:
                ...
                logger_proc.log(
                    3, '[discord] account [' + str(bot_info_dict[bot_hash].id) + '] not hit:\n' + traceback.format_exc()
                )
                continue
        else:
