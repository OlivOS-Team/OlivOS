    ...
    def run(self):
        ...
        self.log(
            ...
            OlivOS.L10NAPI.getTrans(
                ...
                [self.Proc_config['Flask_name'], str(self.Proc_config['Flask_server_port'])],
                modelName,
            ),
        )
        self.log(
        ...
        if self.Proc_config['config'].debug_mode:
            self.Proc_config['Flask_app'].run(
                host=self.Proc_config['Flask_server_host'], port=self.Proc_config['Flask_server_port']
            )
        else:
            server = pywsgi.WSGIServer(
                (self.Proc_config['Flask_server_host'], self.Proc_config['Flask_server_port']),
