    ...
    for message_obj_this in message_obj.data:
        if type(message_obj_this) is OlivOS.messageAPI.PARA.at:
            ...
            if str(message_obj_this.data['id']).isdecimal():
                obj_this['entity'] = {'type': 'mentioned_user', 'user_id': message_obj_this.data['id']}
            else:
                obj_this['entity'] = {'type': 'mentioned_robot', 'bot_id': message_obj_this.data['id']}
...
def get_Event_from_SDK(target_event):
    ...
    if (
        ...
        if type(messageData) is dict:
            ...
            if type(contentData_str) is str and messageData.get('objectName', None) == 'Text':
                ...
                if (
                    ...
                    target_event.data = target_event.group_message(roomId, fromUserId, msg, 'group')
                    target_event.data.message_sdk = OlivOS.messageAPI.Message_templet('olivos_string', msg)
...
class payload_template(object):
    ...
    def load(self):
        if self.raw is not None and len(self.raw) >= 32:
            (self.magicHeader.Magic, self.magicHeader.DataLen) = struct.unpack('<II', self.raw[:8])
            (
                self.dataHeader.HeaderLen,
                self.dataHeader.ID,
                self.dataHeader.Flag,
...
def get_bot_secret(bot_info: bot_info_T):
    return str(
        hmac.new(
            str(bot_info.pub_key).encode('utf-8'), str(bot_info.secret).encode('utf-8'), hashlib.sha256
        ).hexdigest()
    )
...
class inde_interface(OlivOS.API.inde_interface_T):
    ...
    def __create_message(
        target_event, chat_type: str, chat_id: str, object_name: str, content: dict, host_id=None, flag_log: bool = True
    ):
        res_data = None
        if chat_type == 'group':
            res_data = OlivOS.mhyVilaSDK.event_action.create_panel_message(
                target_event=target_event, chat_id=chat_id, object_name=object_name, content=content, host_id=host_id
            )
        return res_data
