...
class server(OlivOS.API.Proc_templet):
    ...
    def on_message(self, ws, message):
        try:
            ...
            if tmp_data_rx_obj.data.s != None:
...
def accountFix(bot_info_dict, logger_proc):
    ...
    for bot_info_dict_this in bot_info_dict:
        ...
        if bot_info_dict[bot_hash].platform['sdk'] == 'discord_link':
            this_msg = OlivOS.discordSDK.API.getMe(OlivOS.discordSDK.get_SDK_bot_info_from_Plugin_bot_info(bot_info_dict[bot_hash]))
            try:
                #刷新至真实bot_id
                ...
                if 'id' in this_msg_res_obj and type(this_msg_res_obj['id']) == str:
                    if this_msg_res_obj['id'].isdigit():
                        logger_proc.log(2, '[discord] account [' + str(bot_info_dict[bot_hash].id) + '] will be updated as [' + str(this_msg_res_obj['id']) + ']')
