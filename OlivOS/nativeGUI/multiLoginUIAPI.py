...
def sendAccountUpdate(obj, control_queue, account_data):
    if control_queue is not None:
        control_queue.put(
            OlivOS.API.Control.packet('call_system_event', {'action': ['account_edit_asayc_end']}), block=False
        )
        control_queue.put(OlivOS.API.Control.packet('call_account_update', {'data': account_data}), block=False)
        control_queue.put(
            OlivOS.API.Control.packet('call_system_stop_type_event', {'action': ['account_update']}), block=False
        )
...
class HostUI:
    ...
    def start(self):
        ...
        self.UIConfig['root_frame_first_root_place'] = {'x': 0, 'y': 0, 'width': 518, 'height': 400}
        ...
        self.UIObject['root_frame_first_root_label_note_new'].configure(
            bg=self.UIConfig['color_001'], fg=self.UIConfig['color_004'], font=('等线', 16, 'bold')
        )
        self.UIObject['root_frame_first_root_label_note_new'].place(
            x=int(518 / 2 - 518 / 2), y=int(400 / 2 / 2 + 20 - 200 / 2), width=518, height=200
        )
        ...
        self.UIObject['root_frame_first_root_label_note_commit'].configure(
            bg=self.UIConfig['color_001'], fg=self.UIConfig['color_004'], font=('等线', 12, 'bold')
        )
        ...
        self.UIConfig['root_frame_skip_root_place'] = {'x': 0, 'y': 0, 'width': 518, 'height': 400}
        ...
        self.UIObject['root_frame_skip_root_label_note_commit'].place(
            x=int(518 / 2 - 518 / 2), y=int(400 / 2 / 2 - 200 / 2), width=518, height=200
        )
...
class TreeEditUI:
    def __init__(self, action, Account_data, hash_key=None, edit_commit_callback=None):
        ...
        self.UIData = {
            ...
            'edit_root_Entry_Extend_list': ['edit_root_Entry_Extend', 'edit_root_Entry_Extend2'],
            ...
            'edit_root_Combobox_dict': {
                ...
                'type_mapping_list_Entry_slot': {
                    ...
                    '黑盒语音': {'机器人ID': 'edit_root_Entry_ID', '机器人令牌': 'edit_root_Entry_Server_access_token'},
                    '米游社/大别野/公域': {
                    ...
                    'B站直播间/游客': {'直播间ID': 'edit_root_Entry_Server_access_token'},
                    'B站直播间/登录': {'直播间ID': 'edit_root_Entry_Server_access_token'},
                    'QQ官方/公域/V1': {
                    ...
                    '接口终端': {'账号': 'edit_root_Entry_ID', '端口': 'edit_root_Entry_Server_port'},
                    'FF14终端': {
                        '账号': 'edit_root_Entry_ID',
                        '端口': 'edit_root_Entry_Server_port',
    ...
    def tree_edit_commit(self):
        ...
        if self.action == 'create' or self.action == 'update':
            ...
            if tmp_platform_platform == 'qqGuild' and tmp_platform_sdk == 'qqGuild_link':
                ...
                if tmp_port == '':
                    tmp_port = '0'
            if tmp_platform_platform == 'qqGuild' and tmp_platform_sdk == 'qqGuildv2_link':
                if tmp_password == '':
                    tmp_password = 'NONEED'
                if tmp_host == '':
                    tmp_host = 'NONEED'
                if tmp_platform_model not in ['public_intents', 'private_intents', 'sandbox_intents']:
                    if tmp_port == '':
                        tmp_port = '0'
            if tmp_platform_platform == 'mhyVila' and tmp_platform_sdk == 'mhyVila_link':
                tmp_id = tmp_id.strip('\n')
                if tmp_host == '':
                    tmp_host = 'NONEED'
                if tmp_port == '':
            ...
            if tmp_platform_platform == 'kaiheila' and tmp_platform_sdk == 'kaiheila_link':
                ...
                if tmp_port == '':
                    tmp_port = '0'
            if tmp_platform_platform == 'xiaoheihe' and tmp_platform_sdk == 'xiaoheihe_link':
                if tmp_password == '':
                    tmp_password = 'NONEED'
                if tmp_host == '':
                    tmp_host = 'NONEED'
            ...
            if not checkByListEmptyOr([
                ...
                type_this = self.get_type_name(
                    tmp_platform_platform, tmp_platform_sdk, tmp_platform_model, tmp_server_auto, tmp_server_type
                )
                if (
                    ...
                    tmp_offset = 0
                    for tmp_Entry_this in self.UIData['edit_root_Combobox_dict']['type_extends_name_note_list'][
                        type_this
                    ]:
                ...
                if (
                    ...
                    for tmp_i in range(self.UIData['edit_root_Entry_qsign_num']):
                        key_pare = ['edit_root_Entry_qsign_addr_%d' % tmp_i, 'edit_root_Entry_qsign_key_%d' % tmp_i]
                        tmp_data = {}
                        tmp_data['addr'] = ''
                        tmp_data['key'] = ''
    ...
    def tree_edit_UI(self):
        ...
        self.tree_UI_Button_init(
            name='edit_root_Button_commit', text='保存', command=self.tree_edit_commit, x=310, y=40, width=70, height=54
        )
    ...
    def tree_edit_UI_Label_init(self, obj_root, obj_name, x, y, width, height, title=''):
        ...
        self.UIObject[obj_name].configure(bg=self.UIConfig['color_001'], fg=self.UIConfig['color_004'])
    ...
    def tree_edit_UI_Entry_init(self, obj_root, obj_name, str_name, x, y, width, height, action, title='', mode='NONE'):
        ...
        self.UIObject[obj_name + '=Label'].place(x=x - 100, y=y, width=100, height=height)
            ...
            if action == 'update':
                ...
                for index in [0, 1, 2, 3, 4]:
                    ...
                    for tmp_list_match_old_this in tmp_list_match_old:
                        if tmp_list_match_old_this != '自定义' and str(tmp_list_data[index]) == str(
            ...
            if tmp_type in self.UIData['edit_root_Combobox_dict']['type_mapping_list']:
                ...
                for entry_this in self.UIData['edit_root_Combobox_dict']['type_mapping_list'][tmp_type][5]:
                    ...
                    self.tree_edit_UI_Entry_init(
                        ...
                        str_name=self.UIData['edit_root_Combobox_dict']['type_mapping_list'][tmp_type][5][entry_this]
                        + '_StringVar',
                        x=100,
                        y=40 + count * (24 + 6),
                        width=200,
                ...
                if (
                    ...
                    for Entry_Extend_this in self.UIData['edit_root_Combobox_dict']['type_extends_name_note_list'][
                        tmp_type
                    ]:
                        if (
                            Entry_Extend_this
                            in (self.UIData['edit_root_Combobox_dict']['type_extends_note_list'][tmp_type])
                        ):
                    ...
                    if con_action == 'qsign_protocal':
                        if action == 'update':
                            if tmp_qsign_protocal in self.UIData['edit_root_Combobox_qsign_protocal_list']:
                                self.UIObject['edit_root_Combobox_qsign_protocal'].current(
                                    self.UIData['edit_root_Combobox_qsign_protocal_list'].index(tmp_qsign_protocal)
                                )
                    tmp_qsign_protocal = self.UIData['edit_root_Combobox_qsign_protocal_StringVar'].get()
                    count += 1
            ...
            if action == 'update':
                if self.hash_key in self.UIData['Account_data']:
                    ...
                    if (
                        ...
                        and dict is type(self.UIData['edit_root_Combobox_dict']['type_extends_note_list'][type_this])
                        and list
                        is type(self.UIData['edit_root_Combobox_dict']['type_extends_name_note_list'][type_this])
                    ):
                        tmp_offset = 0
                        for tmp_Entry_this in self.UIData['edit_root_Combobox_dict']['type_extends_name_note_list'][
                            type_this
                            ...
                            if tmp_offset >= len(self.UIData['edit_root_Entry_Extend_list']):
                                break
                            if (
                                self.UIData['edit_root_Combobox_dict']['type_extends_note_list'][type_this][
                                    tmp_Entry_this
                                ]
    ...
    def get_type_name(self, platform_platform, platform_sdk, platform_model, server_auto, server_type):
        res = None
