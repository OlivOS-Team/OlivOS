...
"""
        ...
        while True:
            ...
            if rx_packet_data.action == 'init':
                ...
                if basic_conf_models_this['enable']:
                    logG(
                        ...
                        OlivOS.L10NAPI.getTrans(
                            'OlivOS model [{0}] will init', [basic_conf_models_this['name']], modelName
                        ),
                    )
                    if basic_conf_models_this['type'] == 'sleep':
                        ...
                        threading.Thread(
                            target=update_get_func, args=(Proc_dict, basic_conf_models, basic_conf_models_this)
                        ).start()
                ...
                main_control.control_queue.put(OlivOS.API.Control.packet('init_type', Proc_type_this), block=False)
                    ...
                    if basic_conf_models_this['type'] == rx_packet_data.key:
                        if Proc_dict[basic_conf_models_this['name']].Proc_info.rx_queue is not None:
                            Proc_dict[basic_conf_models_this['name']].Proc_info.rx_queue.put(
                                OlivOS.API.Control.packet('restart_do', basic_conf_models_this['name']), block=False
                            )
                ...
                if (
                    ...
                    for event_this in rx_packet_data.key['action']:
                        if 'event' in basic_conf['system'] and event_this in basic_conf['system']['event']:
                            for model_this in basic_conf['system']['event'][event_this]:
                                main_control.control_queue.put(main_control.packet('init', model_this), block=False)
                ...
                if (
                    ...
                    for event_this in rx_packet_data.key['action']:
                        if 'type_event' in basic_conf['system'] and event_this in basic_conf['system']['type_event']:
                            for model_this in basic_conf['system']['type_event'][event_this]:
                                main_control.control_queue.put(
                                    main_control.packet('stop_type', model_this), block=False
                                )
                    ...
                    main_control.control_queue.put(
                        main_control.packet(
                            'send',
                            {
                                ...
                                'data': {'action': 'account_update', 'data': plugin_bot_info_dict},
                            },
                        ),
                        block=False,
                    )
                ...
                for tmp_Proc_name in basic_conf_models:
                    ...
                    if basic_conf_models_this['type'] == rx_packet_data.key:
                        main_control.control_queue.put(
                            main_control.packet('init', basic_conf_models_this['name']), block=False
                        )
                        logG(1, OlivOS.L10NAPI.getTrans('OlivOS model [{0}] type init', [tmp_Proc_name], modelName))
            elif rx_packet_data.action == 'stop_type':
                list_stop = []
                for tmp_Proc_name in Proc_Proc_dict:
                    try:
                        if tmp_Proc_name in Proc_dict and rx_packet_data.key == Proc_dict[tmp_Proc_name].Proc_type:
                            Proc_Proc_dict[tmp_Proc_name].terminate()
                            Proc_Proc_dict[tmp_Proc_name].join()
                            list_stop.append(tmp_Proc_name)
                            logG(1, OlivOS.L10NAPI.getTrans('OlivOS model [{0}] will stop', [tmp_Proc_name], modelName))
                    except Exception:
                        traceback.print_exc()
...
def patch_config_by_path(basic_conf: dict, patch_conf: dict, path: list):
    ...
    if len(path) > 0:
        ...
        conf_key = path[-1]
        if conf_key in basic_conf_this and conf_key in patch_conf_this:
            flag_same_type = False
            if type(basic_conf_this[conf_key]) is type(patch_conf_this[conf_key]):
                flag_same_type = True
            elif type(basic_conf_this[conf_key]) in (int, float) and type(patch_conf_this[conf_key]) in (int, float):
                flag_same_type = True
            if flag_same_type:
...
def update_get_func(Proc_dict, basic_conf_models, basic_conf_models_this):
    tmp_model_this_res = OlivOS.updateAPI.OlivOSUpdateGet(logger_proc=Proc_dict[basic_conf_models_this['logger_proc']])
    if tmp_model_this_res:
        for tmp_Proc_name in basic_conf_models:
            ...
            if basic_conf_models_this['type'] == 'plugin':
                if Proc_dict[basic_conf_models_this['name']].Proc_info.rx_queue is not None:
                    Proc_dict[basic_conf_models_this['name']].Proc_info.rx_queue.put(
                        OlivOS.API.Control.packet('update_hit', basic_conf_models_this['name']), block=False
                    )
        ...
        if 'Proc_dict' in varDict and type(varDict['Proc_dict']) is dict:
            ...
            for Proc_name in gMonitorReg['Proc_dict']['keys']:
                if Proc_name not in varDict['Proc_dict']:
                    ...
                    logG(2, OlivOS.L10NAPI.getTrans('OlivOS model [{0}] stopped', [Proc_name], modelName))
            for Proc_name in varDict['Proc_dict']:
                if Proc_name not in gMonitorReg['Proc_dict']['keys']:
                    flagNeedRefresh = True
                    logG(2, OlivOS.L10NAPI.getTrans('OlivOS model [{0}] init', [Proc_name], modelName))
            if flagNeedRefresh:
...
def logG(log_level, log_message, log_segment=None):
    log(logger_proc=gLoggerProc, log_level=log_level, log_message=log_message, log_segment=log_segment)
