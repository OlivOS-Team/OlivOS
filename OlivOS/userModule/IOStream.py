...
def IO_construct(
    ...
    if input_log.get(platform, None) is None:  # 检查文件结构
        input_log[platform] = {}
    if input_log[platform].get('group', None) is None:
        input_log[platform]['group'] = {}
    if input_log[platform].get('private', None) is None:
        input_log[platform]['private'] = {}
    if input_log[platform]['private'].get(user_id, None) is None:
        input_log[platform]['private'][user_id] = {}
    if input_log[platform]['group'].get(group, None) is None:
        input_log[platform]['group'][group] = {}
    if input_log[platform]['group'][group].get(user_id, None) is None:
        input_log[platform]['group'][group][user_id] = {}
    if input_log[platform]['group'][group].get('all', None) is None:
    ...
    if input_index < len(input_list):
        ...
        if input_cache_data.get('input_history_data', None) is None:
            input_cache_data['input_history_data'] = []
        # 将数据传入回调函数运算
        cache_data, new_index = input_func(
            plugin_event=self, string=string, re_string=re_string, cache_data=input_cache_data
        )
