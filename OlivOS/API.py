...
class Event(object):
    ...
    #此处以下定义输入流方法
    def input_construct(self,input_list:'list|tuple',plugin_name,default_mode:int = 2,default_max_time:int = 30,default_func:'function|None' = None):
        mode = default_mode
        ...
        data_path = 'plugin/tmp/Input/{}_input_log.json'.format(plugin_name)
        ...
        if input_index < len(input_list):
            ...
            cache_path = 'plugin/tmp/Input/input_data/{}.json'.format(cache_file_name)
            ...
            with open(cache_path,"r",encoding='utf8') as f:      #导入输入流变量缓存数据
                input_cache_data:dict = json.load(f)
            ...
            if 0 <= new_index < len(input_list):    #新序列合法，不结束输入流，缓存数据
                    ...
                    with open(data_path, "w", encoding='utf8') as f:
                        json.dump(input_log, f, indent=4, ensure_ascii=False)
